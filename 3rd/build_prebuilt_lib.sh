...
cd unrar
make -f ../makefile.unrar CXX=aarch64-linux-android21-clang++ STRIP=aarch64-linux-android-strip AR=aarch64-linux-android-ar DESTDIR=`pwd`/../prebuilt/unrar lib -DUNIX_TIME_NS
mv *.a ../prebuilt/unrar
cd ..


cp linux-syscall-support/lss backpad/src/third_party
